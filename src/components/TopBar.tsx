import { Bell, Search, User as UserIcon, LogOut, Settings, Cast, MonitorPlay, Tv2, Plus, Mic2 } from "lucide-react";
      ...
      <div className="topbar-search-wrapper" ref={searchDropdownRef}>
        ...
        {isFocused && searchValue.trim() && (
          <div className="topbar-search-dropdown animate-fade-in">
            {/* Quick Singer Search Shortcut Header */}
            <div 
              className="topbar-search-singer-shortcut"
              onMouseDown={(e) => {
                e.preventDefault();
                setIsFocused(false);
                navigate(`/artist?name=${encodeURIComponent(searchValue.trim())}`);
              }}
            >
              <Mic2 size={16} color="var(--accent)" />
              <span>Xem bài hát ca sĩ <strong>"{searchValue.trim()}"</strong></span>
            </div>
