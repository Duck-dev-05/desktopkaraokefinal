...
const Login = () => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Add a focus listener to auto-cancel if user returns to the app (e.g. closes browser tab)
      const onAppFocus = () => {
        setTimeout(async () => {
          if (oauthPortRef.current) {
            try {
              await cancel(oauthPortRef.current);
              oauthPortRef.current = null;
            } catch (e) {}
            setIsLoading(false);
            await message("Đăng nhập đã bị hủy do bạn quay lại ứng dụng.", { title: "Đã hủy", kind: "info" });
          }
        }, 1500); // 1.5s grace period to allow redirect to process
        window.removeEventListener('focus', onAppFocus);
      };

      // Delay attaching the listener to ensure the browser has time to steal focus first
      setTimeout(() => {
    ...
    <div className="login-page">
      <div className="login-container">
        <div className="login-header">
          ...
          <h1 className="login-title">KaraokePro</h1>
          <p className="login-subtitle">
            {isLogin ? "Chào Mừng Trở Lại" : "Tạo Tài Khoản"}<br />
        ...
        <form className="login-form" onSubmit={handleSubmit}>
          {!isLogin && (
            <div className="form-group">
              ...
              <input className="form-input" type="text" placeholder="Nhập tên của bạn" required />
            </div>
          )}
          
          <div className="form-group">
            <label className="form-label">Email</label>
            <input className="form-input" type="email" placeholder="Nhập email của bạn" required />
          </div>
          ...
          {isLogin && (
            <div className="login-footer" style={{marginTop: '0', display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
              <label style={{display: 'flex', alignItems: 'center', gap: '8px', cursor: 'pointer', fontSize: '0.85rem'}}>
                <input type="checkbox" /> Ghi nhớ đăng nhập
              </label>
              <a href="#" className="forgot-password" style={{marginLeft: 'auto'}}>Quên mật khẩu?</a>
            </div>
          )}
          ...
          <div className="social-login">
            ...
            >
              ...
              {isLoading ? "Đang chờ trình duyệt..." : "Tiếp tục với Google"}
            </button>
          </div>
        </form>
